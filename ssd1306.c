...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  } else  if (c >= 'a' && c <= 'z')
  {
    index = (c - 'a' + 37) * 8; // Adiciona o deslocamento necessário
        ...
        switch (c) {
            case '.': index = (99) * 8; break;  // Ponto final
            case ',': index = (100) * 8; break; // Vírgula
            case '?': index = (101) * 8; break; // Interrogação
            case '!': index = (102) * 8; break; // Exclamação
            case ':': index = (103) * 8; break; // Dois pontos
            case ';': index = (104) * 8; break; // Ponto e vírgula
            case '(': index = (105) * 8; break; // Abre parêntese
            case ')': index = (106) * 8; break; // Fecha parêntese
            case '*': index = (107) * 8; break; // Asterisco
            default: return; // Caracteres não mapeados são ignorados
        }
  }
...
void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y)
{
  while (*str)
  {
    ...
    while (*str) { // Percorre cada caractere da string
        ...
        str++; // Passa para o próximo caractere da string
    }
  }
}